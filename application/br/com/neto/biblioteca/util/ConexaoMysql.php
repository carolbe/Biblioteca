...
/**
 * Exemplo de uso de uma classe singleton
 *
 * @author Luís Otávio Cobucci Oblonczyk <lcobucci@gmail.com>
 */
...
{
	/**
	 * Imprime na tela os usuarios
	 *
	 * @param string $host
	 */
	public function listaUsuariosPorHost($host)
	{
		$stm = ConexaoMysql::getInstance()->prepare('SELECT * FROM user WHERE Host = ?');
		$stm->execute(array($host));

		while ($produto = $stm->fetchObject()) {
			echo $produto->User . '@' . $produto->Host . '<br />' . "\n";
		}
	}
}

$testeSingleton = new TesteSingleton();
$testeSingleton->listaUsuariosPorHost('localhost');